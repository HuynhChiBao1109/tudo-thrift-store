              ...
              <tbody>
                {products.map(product => (
                  <tr key={product.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={cn('text-xs px-2.5 py-1 rounded-full font-medium', getConditionColor(product.condition))}>
                        {getConditionLabel(product.condition)}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-gray-600">{product.size}</td>
                    <td className="px-6 py-4 font-semibold text-[#06365b]">{formatPrice(product.price)}</td>
                    <td className="px-6 py-4">
                      <span className={cn('text-xs font-medium', product.stock === 0 ? 'text-red-500' : 'text-gray-700')}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-1.5 text-gray-400 hover:text-[#06365b] hover:bg-blue-50 rounded transition-colors"
                        >
                          <Edit2 size={14} />
                        </button>
                        <button
