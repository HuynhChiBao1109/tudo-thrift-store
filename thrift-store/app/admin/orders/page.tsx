        ...
        {statuses.map(status => {
          ...
          return (
            <div key={status} className="bg-white rounded-xl p-4 border border-gray-100">
              ...
              <p className={cn('text-xs mt-1 font-medium capitalize px-2 py-0.5 rounded-full inline-block', getStatusColor(status))}>
                {status}
              </p>
            </div>
          );
              ...
              <tbody>
                {orders.map(order => (
                  <tr key={order.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4 font-mono text-xs text-[#06365b] font-medium">{order.id}</td>
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        <SelectTrigger className={cn('w-32 h-7 text-xs border-0 rounded-full px-3', getStatusColor(order.status))}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {statuses.map(s => (
